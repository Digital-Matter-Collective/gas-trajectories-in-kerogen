...
class DistanceMatrixParams:
    ...
    """ Critical value for the invariant (1 is perfect square)
        ...
        Publication values are supplied by an explicit optimized profile;
        the dataclass default is not a publication configuration.
    """
    nu: float = 0.1
...
class DistanceMatrixAnalyzer(TrajectoryAnalyzer):
    """Classify trajectory steps using point labels from a distance matrix.

    DM natively assigns a trapped/free label to each of the ``N`` trajectory
    points. The common analyzer interface requires ``N - 1`` edge labels. For
    an edge ``x_i -> x_{i+1}``, this implementation intentionally uses the
    label of its destination point ``x_{i+1}``.
    """
    ...
    def run(self, trj: Trajectory) -> npt.NDArray[np.bool_]:
        ...
        for flag, _, result in results:
            if flag:
                list_trapped |= result
        return self.point_labels_to_edge_labels(list_trapped)

    @staticmethod
    def point_labels_to_edge_labels(point_labels: NPBArray) -> NPBArray:
        """Map point labels to edges using the destination-point convention."""
        labels = np.asarray(point_labels, dtype=np.bool_)
        if labels.ndim != 1:
            raise ValueError(
                f"Expected a one-dimensional point-label array, got {labels.shape}"
            )
        return labels[1:]
    ...
    def analyse_by_mu(
        ...
        if len(List_min_max) == 0:
            return (True, invariant, list_trapped_m)
